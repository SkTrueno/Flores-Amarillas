...
fuente_frase = pygame.font.SysFont("arial", 25, bold=True)
...
class ParticulaCorazon:
    def __init__(self, x, y):
        self.x = x
        self.y = y
        self.tamano = random.choice([1, 2, 2, 3])
        self.brillo = random.randint(150, 255)
        self.velocidad = random.uniform(0.5, 2)
        self.offset = random.uniform(0, math.pi * 2)

    def actualizar(self, tiempo):
        movimiento = math.sin(tiempo * 2 + self.offset) * 0.8
        self.y += movimiento * 0.02

    def dibujar(self):
        intensidad = max(100, min(255, int(self.brillo)))
        color = (255, intensidad, 20)
        pygame.draw.circle(pantalla, color, (int(self.x), int(self.y)), self.tamano)

particulas = [ParticulaCorazon(x, y) for x, y in particulas_corazon]
...
class ParticulaFondo:
    def __init__(self):
        ...
        self.velocidad = random.uniform(0.2, 1.2)
        self.brillo = random.randint(80, 220)
    ...
    def dibujar(self):
        color = (255, random.randint(170, 230), 40)
        pygame.draw.circle(pantalla, color, (int(self.x), int(self.y)), self.tamano)

particulas_fondo = [ParticulaFondo() for _ in range(250)]

# ============================================================
# CORAZONES DEL FONDO
# ============================================================

def dibujar_corazon_pequeno(superficie, x, y, tamano, color):
    puntos = []
    for i in range(40):
        t = math.pi * 2 * i / 40
        hx = 16 * math.sin(t) ** 3
...
class Flor:
    ...
    def dibujar(self, tiempo):
        movimiento = math.sin(tiempo * 1.5 + self.fase) * 4
        x = self.x
        y = self.y + movimiento
        ...
        pygame.draw.line(
            pantalla,
            (60, 130, 45),
            (int(x), int(y + 10 * self.escala)),
            (int(x), int(y + 55 * self.escala)),
            max(1, int(3 * self.escala))
        )
...
def dibujar_titulo(tiempo):
    brillo = (math.sin(tiempo * 3) + 1) / 2
    intensidad = int(180 + brillo * 75)
    color_brillo = (255, intensidad, 100)

    texto_sombra = fuente_titulo.render("Te Amo", True, (100, 70, 0))
    rect_sombra = texto_sombra.get_rect(center=(ANCHO // 2 + 3, 85 + 3))
    pantalla.blit(texto_sombra, rect_sombra)

    texto = fuente_titulo.render("Te Amo", True, color_brillo)
    rect = texto.get_rect(center=(ANCHO // 2, 85))
...
def dibujar_frase():
    ...
    texto = fuente_frase.render(frase, True, (255, 240, 150))
    rect = texto.get_rect(center=(ANCHO // 2, ALTO - 55))
    pantalla.blit(texto, rect)
...
async def main():
    global indice_frase, tiempo_frase
    ...
    while ejecutando:
        ...
        if tiempo_frase >= 3.5:
            tiempo_frase = 0
            indice_frase = (indice_frase + 1) % len(frases)
        ...
        for corazon in corazones_fondo:
            ...
            brillo = int(20 + 20 * (math.sin(tiempo_total + corazon["fase"]) + 1))
        ...
        dibujar_titulo(tiempo_total)

        for particula in particulas:
            particula.actualizar(tiempo_total)
            particula.dibujar()

        for flor in flores:
            flor.dibujar(tiempo_total)
